...
ui <- dashboardPage(
  ...
  dashboardBody(
    tabItems(
      tabItem(
        tabName = "plot_tab", fluid = TRUE,
        fluidRow(
          valueBox("Unread Mail", 44, icon("mail"), color = "blue", width = 5)),
        fluidRow(
          box(
            title = "Sample box", color = "blue", width = 11,
            selectInput(
              inputId =  "variable1",
          ...
          tabBox(
            title = "Sample box", color = "blue", width = 5,
            collapsible = FALSE,
            tabs = list(
...
server <- function(input, output) {
  ...
  output$mtcars_plot <- renderPlotly(
    plot_ly(
      mtcars, x = ~ mtcars[ , input$variable1],
