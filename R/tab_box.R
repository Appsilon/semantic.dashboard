...
#' @param ... Tabs to include within the box.
...
tab_box <- function(tabs,  title = NULL, color = "", ribbon = TRUE, title_side = "top right", collapsible = TRUE, width = 8){
  box(tabs, title = title, color = color, ribbon = ribbon, title_side = title_side, collapsible = collapsible, width = width)
}

#' @describeIn tab_box Create a tab box.
#' @export
tabBox <- tab_box
